...
function TypeModal({ show, onHide, type, onSaved }) {
  ...
  const removeProp = (index) => {
    setSelectedProps((prev) => prev.filter((_, i) => i !== index));
  };

  const setRequired = (index, required) => {
    setSelectedProps((prev) =>
      prev.map((p, i) => (i === index ? { ...p, required } : p))
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (type) {
        await api.updateLocationType(type.uuid, payload);
      } else {
        await api.createLocationType(payload);
      }
      onSaved();
      onHide();
    } catch (err) {
        ...
        <div className="modal-content">
          <form onSubmit={handleSubmit}>
            <div className="modal-header">
              <h5 className="modal-title">Locatietype Configureren</h5>
            ...
            <div className="modal-body">
              ...
              <div className="input-group">
                <select id="propertyLibrarySelect" className="form-select">
