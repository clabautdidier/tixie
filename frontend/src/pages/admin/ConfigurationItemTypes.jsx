...
function TypeModal({ show, onHide, type, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (type) await api.updateConfigurationItemType(type.uuid, payload);
      else await api.createConfigurationItemType(payload);
      onSaved();
      onHide();
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <form onSubmit={handleSubmit}>
          <div className="flex items-center justify-between p-4 border-b">
            <h3 className="text-lg font-semibold">CI Type Configureren</h3>
            <button type="button" className="text-gray-400 hover:text-gray-600 text-2xl" onClick={onHide}>×</button>
          </div>
          <div className="p-4 space-y-4 overflow-auto">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Naam *</label>
              <input type="text" className={inputClass} placeholder="bijv. LAPTOP" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} required />
            </div>
            <div>
            ...
            <hr />
            <h4 className="font-medium">Eigenschappen uit Bibliotheek</h4>
            <p className="text-sm text-gray-500">Selecteer welke velden bij dit type horen.</p>
            ...
            <div className="flex gap-2">
              <select id="propertyDefinitionSelect" className="flex-1 px-3 py-2 border border-gray-300 rounded-md">
...
export default function ConfigurationItemTypes() {
  ...
  const load = async () => { const data = await api.getConfigurationItemTypes(); setTypes(data); };
  useEffect(() => { load(); }, []);
  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Configuration Item Types</h1>
          <p className="text-gray-500 mt-1">Definieer blauwdrukken voor assets (bijv. Laptop, Server).</p>
        </div>
